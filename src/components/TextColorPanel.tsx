...
import { X, ArrowLeft, Palette, RotateCcw } from 'lucide-react';
...
const TextColorPanel: React.FC<TextColorPanelProps> = ({ 
  isOpen, 
  onClose,
  selectedTextId,
  currentColor
}) => {
  const { textElements, updateTextElement } = useTextContext();
  ...
  useEffect(() => {
    if (selectedElement) {
      setSelectedColor(selectedElement.color || '#000000');
      setCustomColor(selectedElement.color || '#000000');
      const hsv = hexToHsv(selectedElement.color || '#000000');
      const cmyk = hexToCmyk(selectedElement.color || '#000000');
      setHsvValues(hsv);
      setCmykValues(cmyk);
    }
  }, [selectedElement]);
    ...
    <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100 h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Text Color</h2>
        </div>
        <button
          onClick={onClose}
