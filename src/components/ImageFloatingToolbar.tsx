...
const ImageFloatingToolbar: React.FC<ImageFloatingToolbarProps> = ({ 
  ...
  useEffect(() => {
    if (selectedElement && selectedElement.filter) {
      ...
      const filterMatch = selectedElement.filter.match(/hue-rotate\(([^)]+)\)|saturate\(([^)]+)\)|brightness\(([^)]+)\)|contrast\(([^)]+)\)/g);
      if (filterMatch) {
        filterMatch.forEach(filter => {
          if (filter.includes('hue-rotate')) {
            const value = parseFloat(filter.match(/\d+/)?.[0] || '0');
            setHue(value);
          } else if (filter.includes('saturate')) {
            const value = parseFloat(filter.match(/[\d.]+/)?.[0] || '1');
            setSaturation(value);
          } else if (filter.includes('brightness')) {
            const value = parseFloat(filter.match(/[\d.]+/)?.[0] || '1');
            setBrightness(value);
          } else if (filter.includes('contrast')) {
            const value = parseFloat(filter.match(/[\d.]+/)?.[0] || '1');
            setContrast(value);
          }
        });
  ...
  const applyFilter = (newHue = hue, newSaturation = saturation, newBrightness = brightness, newContrast = contrast) => {
    if (selectedElement) {
      const filterString = `hue-rotate(${newHue}deg) saturate(${newSaturation}) brightness(${newBrightness}) contrast(${newContrast})`;
